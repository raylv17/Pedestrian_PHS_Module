...
function agent_step!(agent::Pedestrian, model, num_solver)
    ...
    model.kinetic = calc_kinetic(model)
    model.potential = calc_potential(model)
...
function initialize(number_of_peds::Int64 = 32, x_len::Real = 11, y_len::Real = 5, num_solver=euler_step, 
    ...
    if isnothing(properties)
        properties = Dict(:λ => 2, :A => 5, :B => 0.3,
                          :kinetic => 0.0, :potential => 0.0, 
                          :hamiltonian => 0.0, :dH => 0.0, :no_disp_H => 0.0, 
                          :dt => 0.01, :sigma => 0.1, :alignment => 0.0, :stoch_dH => 0.0)
    end
    rng = Xoshiro(seed)
